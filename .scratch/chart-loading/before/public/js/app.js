...
function sanitizeCandles(list, maxLimit = 3000) {
  ...
  // Auto-detect timeframe interval of the candle series for contiguous gap filling
  let tfMs = (typeof activeTf !== "undefined" && TF_MS[activeTf]) ? TF_MS[activeTf] : 60000;
  if (out.length >= 2) {
    const diffs = [];
    for (let i = 1; i < Math.min(out.length, 30); i++) {
      const d = out[i].t - out[i - 1].t;
      if (d > 0 && d <= 86400000) diffs.push(d);
    }
    if (diffs.length) {
      diffs.sort((a, b) => a - b);
      const medianDiff = diffs[Math.floor(diffs.length / 2)];
      if (medianDiff >= 1000) tfMs = medianDiff;
    }
  }

  // Fill small time gaps within contiguous series so chart has no missing bars/gaps
  const filled = [];
  for (let i = 0; i < out.length; i++) {
    if (i > 0) {
      ...
      if (dt > tfMs * 1.5 && dt <= tfMs * 50) {
        const missingCount = Math.round(dt / tfMs) - 1;
  ...
  const cleaned = cleanPhantomWicksAndCandles(filled);
...
async function loadOlderHistory(ex, sym, tf) {
  ...
  try {
    ...
    try {
      const r = await fetch(`/api/klines?ex=${encodeURIComponent(ex)}&sym=${encodeURIComponent(sym)}&tf=${encodeURIComponent(tf)}&before=${endTime}`, { signal: controller.signal });
      if (r.ok) {
        const data = await r.json();
        if (Array.isArray(data) && data.length > 0) {
          if (typeof data[0] === 'number') {
...
function appendCandle(k) {
  ...
  if (clean.t === last.t) {
    ...
    if (gap > 5) {
      // Large gap — likely a reconnect. Skip flat placeholders (they appear as "sticks").
      // Immediately refetch real history to fill the gap with actual OHLCV data.
      window._isRefetchingGap = false; // reset lock so refetch runs immediately
      refetchMissingHistory(activeEx, activeSym, activeTf);
    } else if (gap > 1) {
      // Small gap (2–5 bars) — normal for illiquid pairs. Add flat placeholders.
      for (let b = 1; b < gap; b++) {
        candles.push({
          t: last.t + b * tfMs,
          o: last.c,
    ...
    if (offsetX > 0) offsetX = getClampedOffsetX(offsetX + (gap > 1 ? Math.min(gap, 100) : 1));
    clearCandleCaches(candles);
  } else {
    // If clean.t < last.t:
...
class ChartInstance {
  ...
  async loadKlines() {
    ...
    if (cached && Date.now() - cached.ts < (typeof KLINES_CACHE_TTL_MS !== "undefined" ? KLINES_CACHE_TTL_MS : 300000) && Array.isArray(cached.data) && cached.data.length > 1) {
      let candList = [];
    ...
    try {
      // 2. Ultra-fast screener-style parallel racer: Direct Exchange REST API + Server Proxy in parallel!
      // Direct exchange CDN takes only ~20-50ms and bypasses node-server connection bottlenecks completely.
      const directPromise = fetchDirectKlines(this.ex, this.sym, this.tf).then(c => (c && c.length > 1 ? c : Promise.reject()));
      const serverPromise = fetchServerKlines(this.ex, this.sym, this.tf, 1).then(c => (c && c.length > 1 ? c : Promise.reject()));

      let fastCandles = [];
      try {
        fastCandles = await Promise.any([directPromise, serverPromise]);
      } catch (_) {
        fastCandles = await fetchServerKlines(this.ex, this.sym, this.tf, 0);
      }

      if (this._loadToken !== myToken) return;

      if (Array.isArray(fastCandles) && fastCandles.length > 1) {
        this.candles = sanitizeCandles(fastCandles);
        this.levels = window.detectChartLevelsFn(this.candles);
        if (activeView === 'formations') window.registerFormationsCoinLevels?.(this.ex, this.sym, this.levels);
        KLINES_CACHE.set(key, { ts: Date.now(), data: this.candles });
        this.loadingKlines = false;
        this.draw(true);
      }
  ...
  async loadOlderHistory() {
    ...
    try {
      let olderCandles = [];
      const r = await fetch(`/api/klines?ex=${encodeURIComponent(curEx)}&sym=${encodeURIComponent(curSym)}&tf=${encodeURIComponent(curTf)}&before=${endTime}`);
      if (r.ok) {
        const data = await r.json();
        if (Array.isArray(data) && data.length > 0) {
          if (typeof data[0] === 'number') {
            ...
            olderCandles = sanitizeCandles(flat, 10000);
          } else {
            olderCandles = sanitizeCandles(data, 10000);
          }
        }
      }

      if (this.sym !== curSym || this.ex !== curEx || this.tf !== curTf) return;
      ...
      const added = this.candles.length - prevLen;
      if (added > 0 && this.offsetX > 0) {
        this.offsetX += added;
      }
      this.refreshFormationLevels(true);
      this.draw(true);
    } catch (e) {
      console.warn(`[ChartInstance] loadOlderHistory error:`, e);
      this.hasReachedStart = true;
    } finally {
      this.loadingOlder = false;
    }
      ...
      const promises = batch.map(async (c) => {
        ...
        try {
          ...
          if (cached && Date.now() - cached.ts < 300000) {
            ...
            try {
              const directP = fetchDirectKlines(c.ex, c.sym, tf).then(d => (d && d.length > 1 ? d : Promise.reject()));
              const serverP = fetchServerKlines(c.ex, c.sym, tf, 1).then(s => (s && s.length > 1 ? s : Promise.reject()));
              klinesData = await Promise.any([directP, serverP]);
              if (Array.isArray(klinesData) && klinesData.length > 0) {
                KLINES_CACHE.set(key, { ts: Date.now(), data: klinesData });
              }
            } catch (e) { }
          }
