  ...
  if (ex === "BN") {
    ...
    getKuCoinToken().then(tk => {
      ...
      sub.ws.on("open", () => {
        ...
        sub.ws.send(JSON.stringify({ id: Date.now(), type: "subscribe", topic: `/contractMarket/limitCandle:${sym}_${periods[tf] || "1hour"}`, privateChannel: false, response: true }));
      ...
      sub.ws.on("message", (raw) => {
        try {
          ...
          if (d.subject === "candle.stick" && Array.isArray(d.data?.candles)) {
            ...
            broadcastKline(ex, sym, tf, { t: +k[0] * 1000, o: +k[1], h: +k[3], l: +k[4], c: +k[2], v: +k[6] });
...
function findLinkedJournalCredentials(userId, exchange) {
  let credentials = journalCredentials.get(userId, exchange);
  if (credentials) return credentials;

  const allUsers = typeof userStore.getAllUsersRaw === "function" ? userStore.getAllUsersRaw() : {};
  const currentUser = allUsers[userId];
  const adminChatId = String(process.env.ADMIN_CHAT_ID || "").trim();

  const candidates = Object.values(allUsers).filter(u => {
    if (!u || !u.id || u.id === userId) return false;
    if (currentUser?.telegramId && (u.telegramId === currentUser.telegramId || u.telegramChatId === currentUser.telegramId)) return true;
    if (currentUser?.telegramChatId && (u.telegramChatId === currentUser.telegramChatId || u.telegramId === currentUser.telegramChatId)) return true;
    if (adminChatId && (currentUser?.telegramId === adminChatId || currentUser?.telegramChatId === adminChatId || currentUser?.role === "VIP Trader" || currentUser?.role === "admin")) {
      if (u.telegramId === adminChatId || u.telegramChatId === adminChatId) return true;
    }
    if (currentUser?.lastIp && u.lastIp === currentUser.lastIp && (currentUser.role === "VIP Trader" || currentUser.role === "admin" || u.role === "VIP Trader" || u.role === "admin")) {
      return true;
    }
    return false;
  });
  ...
  for (const cand of candidates) {
    const found = journalCredentials.get(cand.id, exchange);
    if (found) {
      try { journalCredentials.save(userId, exchange, found); } catch (_) {}
      return found;
    }
  }
  return null;
...
function findLinkedJournalExchanges(userId) {
  let exchanges = journalCredentials.list(userId);
  ...
  for (const cand of candidates) {
    const list = journalCredentials.list(cand.id);
    if (list && list.length > 0) {
      for (const item of list) {
        const creds = journalCredentials.get(cand.id, item.exchange);
        if (creds) {
          try { journalCredentials.save(userId, item.exchange, creds); } catch (_) {}
        }
      }
...
app.get("/api/journal/credentials", (req, res) => {
  ...
  return res.json({ success: true, exchanges: findLinkedJournalExchanges(user.id) });
});
