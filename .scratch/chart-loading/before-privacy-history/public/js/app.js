...
async function fetchChartKlines(ex, sym, tf) {
  ...
  const request = (async () => {
    ...
    const nonempty = data => data?.length > 1 ? data : Promise.reject(new Error('No candles yet'));
...
function fetchGridKlines(ex, sym, tf) {
  ...
  if (!group) {
    ...
    setTimeout(() => {
      GRID_KLINE_QUEUE.delete(groupKey);
      flushGridKlines(group);
    }, 0);
  }
  const request = new Promise(resolve => group.items.set(sym, resolve));
  KLINE_REQUESTS.set(key, request);
  request.finally(() => { if (KLINE_REQUESTS.get(key) === request) KLINE_REQUESTS.delete(key); });
  return request;
}
...
async function flushGridKlines(group) {
  ...
  const consume = line => {
    ...
    if (data.length > 1 && pending.has(row.sym)) {
  ...
  try {
    ...
    clearTimeout(timer);
    controller.abort();
    for (const [sym, resolve] of pending) {
...
class ChartInstance {
  ...
  async loadKlines() {
    ...
    try {
      ...
      const fastCandles = await fetchGridKlines(this.ex, this.sym, this.tf);
